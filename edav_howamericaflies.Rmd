---
title: "EDAV Project [Fall 2018]: How America Flies ?"
author: ""
date: ""
output: 
...
year_data <- read_csv('data/01_year_data.csv')
data <- read_csv('data/jan2018.csv')
december_data <- read_csv('data/02_december_data.csv')
...
Please find our github repository at for the project at: https://github.com/ameyakarnad/howamericaflies
...
- The data that we are using for our data analysis and visualisation is taken from United States Department of Transportation's Bureau of Transportation Statistics (BTS). 
  Link : https://catalog.data.gov/dataset/airline-on-time-performance-and-causes-of-flight-delays-on-time-data
- The collected data is sampled from the period of October 2017 to September 2018. We restricted purself to this timescale because the dataset is very large and it gives trouble after loading in R to do quick operatoins. 
...
```{r, echo=FALSE, warning= FALSE, fig.width=20,fig.height=10, warning= FALSE}

air_data <- year_data

# Top airlines with respect to Number of flights
total_airline <-  air_data %>%
  select(Reporting_Airline, Origin, Dest) %>%
  group_by(Reporting_Airline) %>%
  summarise(count = n()) %>%
  arrange(desc(count))

colours_rank <-  c("#1a3c2c","#563622","#da419e","#a89924","#7b3f22","#a8685e","#045280","#b2202e","#b73b5c",
                   "#244748","#abe138","#223520","#c358f8","#ec8030","#01124c","#aadfe1","#8c3d01","#a8685e")



ggplot(data = total_airline, mapping = aes(x = reorder(Reporting_Airline, desc(count)),  y = count)) + 
  geom_histogram(stat = "identity", fill = colours_rank)+
  theme(axis.ticks = element_blank(),plot.title = element_text(size = rel(1.2))) +
  labs(x = "Airlines", y = "Number of flights operated",
       title = "Number of Flights operated by Domestic Airlines in 2017",
       subtitle = "Southwestern Airlines leads the way")
...
# Number of unique reoutes foloowed by airlines
unique_routes <- distinct(air_data %>%
  select(Reporting_Airline, Origin, Dest) %>%
  group_by(Reporting_Airline)) 
...
colours_routes <-  c("#a89924","#1a3c2c","#563622",
                     "#045280","#8c3d01","#7b3f22",
                     "#b73b5c","#da419e","#01124c",
                     "#223520","#244748","#c358f8",
                     "#abe138","#ec8030","#a8685e",
                     "#b2202e","#a8685e","#aadfe1")


ggplot(data = total_routes, mapping = aes(x = reorder(Reporting_Airline, desc(count)),  y = count)) + 
  geom_histogram(stat = "identity", fill = colours_routes)+
  theme(axis.ticks = element_blank(),plot.title = element_text(size = rel(1.2))) +
  labs(x = "Airlines", y = "Number of routes operated",
       title = "Number of routes operated by Domestic Airlines in 2017",
       subtitle = "The competition seems much closer, SkyWest has the edge")
...
arr_delay$DAY_OF_WEEK <- recode(arr_delay$DAY_OF_WEEK,
  "1" = "Monday" ,"2" = "Tuesday","3" = "Wednesday","4" = "Thursday","5" = "Friday",
    "6" = "Saturday","7" = "Sunday")
d2 <- ggplot(arr_delay, aes(x = as.numeric(CRS_ARR_TIME), 
                            y = DAY_OF_WEEK, 
                            group = DAY_OF_WEEK)) + 
...
- American Airlines (AA) suffers lots of cancellations due to carrier realted and weather issue at Charlotte (CLT), Dallas (DFW) and Chicago(ORD) which also happen to be its hub airports.
- In a contrast to AA, domestic airline Skywest has cancellations at its hub in Chicago but majorly due to weather and not due to its own problem.
...
- As we know, most of the cancellation happen due to weather conditions
...
- There were a lot of cancellations in the flight in the year 2013 in early december.
- 2015 was a bad year as flights were cancelled during the holiday season. 
...
- 2016 was a surprising year. The flight cancellations due to NAS drastically decreased in this period. This is probably because the number of flights operations at JFK were restricted and enforced in the year 2016, which probably decreased the number of flights cancellations due to NAS. 
Link: https://www.federalregister.gov/documents/2016/06/21/2016-14631/operating-limitations-at-john-f-kennedy-international-airport
...
-	The dictionary was then used in the d3 script to show flights and delays
Vision
Websites like Google flights and Expedia are quite commonly used to book flight tickets today. But the websites only tell the customer about the flight timings and cost of travel.  The customer has no knowledge about the historical delays that the flight has on the particular route. So, we decided to design a website which gives the user information about the delay in flights and its patterns so that he/she can make an informed choice
